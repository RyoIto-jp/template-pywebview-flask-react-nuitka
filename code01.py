...
if __name__ == '__main__':
    ...
    if params['isSelf']:
        ...
        result = get_worktime(page.find('table', id="APPROVALGRD"), TARGET_YEAR, isReported)

        result = [{**row, "Name": dispName, "Num": USER_NAME,
                "Report": reportStatus} for row in result]

        output_path = f"result/aa_{str(TARGET_YEAR)[2:]}{str(TARGET_MONTH).zfill(2)}_{USER_NAME}.csv"
        report_csv(output_path, result, USER_NAME, dispName)
    ...
    for member in params['members']:
        ...
        member_info, dispName, reportStatus = search_request(search_page, LOGIN_URL, TARGET_YEAR, TARGET_MONTH, member)
        worktime_table = member_worktime_request(member_info)

        isReported = False if reportStatus=="状態：未入力" else True
        result = get_member_worktime(worktime_table, TARGET_YEAR, isReported)
        result = [{**row, "Name": dispName, "Num": member,
                "Report": reportStatus} for row in result]

        output_path = f"result/aa_{str(TARGET_YEAR)[2:]}{str(TARGET_MONTH).zfill(2)}_{member}.csv"
